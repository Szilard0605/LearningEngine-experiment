...
void main()
{
	...
	//testing light
	//float intensity = 1.0 / length(inPosition.xy - light_pos) * light_intensity;
	
	//float radius = 350.0;
	float ldistance  = length( light_pos - inPosition.xy );

    float maxDistance = pow( light_radius, 0.20);
    float quadDistance = pow( ldistance, 0.23);

    float quadIntensity = 1.0 - min( quadDistance, maxDistance )/maxDistance;
	

	texColor.x *= quadIntensity * light_intensity;
	texColor.y *= quadIntensity * light_intensity;
	texColor.z *= quadIntensity * light_intensity;*/
	o_color = texColor;
}